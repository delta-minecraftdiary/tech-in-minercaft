...
with open("sources/inventory/1.16.5/creative_tab.json", mode="r", encoding="UTF-8") as tab_file:
    tabs_json = json.load(tab_file)

ver = "1.16.5"

def get_group(item):
    for tab in tabs_json["tabs"]:
        if item in tab: return tab.keys()


def get_image(item):
    if item == "golden apple": return "golden apple.gif"
    else: return "{}.png".format(item)
